...
public class Ferry {
    ...
    private static final List<Character> MOVE_INSTRUCTIONS = Arrays.asList('N', 'S', 'E', 'W', 'F');
    ...
    private void interpretMoveInstruction(char instruction, int amount) {
        switch (instruction) {
            ...
            case 'F': System.out.print("Moving forward: ");
                interpretMoveInstruction(facing, amount);
                break;
    ...
    private void turn(int direction, int deg) {
        ...
        int num90DegTurns = deg / DEG_90;
        int directionIndex = DIRECTIONS.indexOf(facing);

        for (int i = 0; i < num90DegTurns; i++) {
            // Add DIRECTIONS.size before using remainder to act as modulus
            directionIndex = (directionIndex + direction + DIRECTIONS.size()) % DIRECTIONS.size();
        }

        facing = DIRECTIONS.get(directionIndex);
    }
}
